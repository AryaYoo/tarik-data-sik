...
class PenerimaanObatFarmasiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('tgl_mulai')) {
            ...
            ExtractionLog::create([
                ...
                'extraction_type' => 'Penerimaan Obat & BHP Farmasi',
            ]);
            ...
            $baseQuery = $this->farmasiRepository->getPenerimaanQuery($tgl_mulai, $tgl_selesai);
        ...
        return view('farmasi.penerimaan_obat.index', compact('data', 'tgl_mulai', 'tgl_selesai', 'top3', 'sort', 'direction'));
    }
    ...
    public function exportExcel(Request $request)
    {
        $tgl_mulai = $request->tgl_mulai ?? date('Y-m-01');
        $tgl_selesai = $request->tgl_selesai ?? date('Y-m-t');
        $data = $this->farmasiRepository->getPenerimaanQuery($tgl_mulai, $tgl_selesai)
            ->orderBy('barang', 'asc')
            ->get();

        return Excel::download(new \App\Exports\PenerimaanObatFarmasiExport($data), 'rekap-penerimaan-obat-farmasi-' . now()->format('Y-m-d') . '.xlsx');
    }
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('farmasi.penerimaan_obat.pdf', compact('data', 'tgl_mulai', 'tgl_selesai'));
        return $pdf->download('rekap-penerimaan-obat-farmasi-' . now()->format('Y-m-d') . '.pdf');
    }
}
